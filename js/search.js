...
const fetchAllSpotifyData = async (urls, token) => {
  ...
  searchList = [...trackItems, ...playlistItems, ...albumItems, ...artistItems];
  renderSearchResult();
...
export const callSpotifyAPI = async (query) => {
...
const renderSearchResult = () => {
  ...
  if (trackHTML) {
    trackHTML = `<div class="search-track"><h1>Track</h1>${trackHTML}</div>`;
  }
  if (playlistHTML) {
    playlistHTML = `<div class="search-playlist"><h1>Playlist</h1>${playlistHTML}</div>`;
  }
  if (albumHTML) {
    albumHTML = `<div class="search-album"<h1>앨범</h1>${albumHTML}</div>`;
  }
  if (artistHTML) {
    artistHTML = `<div class="search-artist"><h1>Artist</h1>${artistHTML}</div>`;
  }
